...
#define CLKSRCSEL_Val         0x00000001 // EXT_OSC -> PLL
#define PLL0_SETUP            1
#define PLL0CFG_Val           0x00050063 // PLL = 400 MHz, CCLK = 100 MHz
//#define PLL0CFG_Val           0x0000000B // PLL = ??? MHz, CCLK = 72 MHz
#define PLL1_SETUP            1
...
void SystemInit (void)
{
  ...
  if (((SC->PLL0STAT >> 24) & 3) == 3) {/* If PLL0 enabled and connected      */
    ...
    SystemFrequency /= (SC->CCLKCFG & 0xFF) + 1; // Divide por CCLK
    SystemFrequency *= 2 * ((SC->PLL0STAT & 0x7FFF) + 1); // Multiplica por 2 * M
    SystemFrequency /= ((SC->PLL0STAT >> 16) & 0xFF) + 1; // Divide por N
  } else {
    switch (SC->CLKSRCSEL & 0x03) {
      case 0:                           /* Internal RC oscillator => PLL0     */
      case 3:                           /* Reserved, default to Internal RC   */
