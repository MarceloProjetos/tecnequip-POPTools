...
void PLC_Run(void)
{
    ...
    /* iniciando serie [ */
    if (((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        ArrayIntUser[3] = MODBUS_REGISTER[0]; // Valor = ValorMB
    }
    ...
    /* iniciando serie [ */
    if (((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        if (ArrayIntUser[4] < 299) {
            ArrayIntUser[4]++;
            ArrayBitSystem[0] &= ~(1UL << 1); // $rung_top = 0
        }
    } else {
        ArrayIntUser[4] = 0; // EsperaON
    }
    ...
    if (((ArrayBitSystem[0] >> 6) & 1)) {  // $parThis_0000
        if (!((ArrayBitSystem[0] >> 7) & 1)) {  // $oneShot_0001
            if (I_SerialReady) {  // $SerialReady
                Modbus_Send(0, 0UL, MODBUS_FC_WRITE_MULTIPLE_REGISTERS, 0, 1, &ArrayIntUser[3]);
    ...
    if (((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        if (!((ArrayBitSystem[0] >> 11) & 1)) {  // $oneShot_0003
            if (I_SerialReady) {  // $SerialReady
                Modbus_Send(0, 0UL, MODBUS_FC_READ_HOLDING_REGISTERS, 0, 1, &ArrayIntUser[5]);
    ...
    if (((ArrayBitSystem[0] >> 6) & 1)) {  // $parThis_0000
        ArrayIntUser[5] = ArrayIntUser[5] + ArrayIntUser[2]; // SaidasPOP1 = SaidasPOP1 + Incremento
    }
    ...
    if (((ArrayBitSystem[0] >> 6) & 1)) {  // $parThis_0000
        if (!((ArrayBitSystem[0] >> 14) & 1)) {  // $oneShot_0006
            if (I_SerialReady) {  // $SerialReady
                Modbus_Send(1, 0UL, MODBUS_FC_WRITE_MULTIPLE_REGISTERS, 0, 1, &ArrayIntUser[5]);
    ...
    if (((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        if (!((ArrayBitSystem[0] >> 16) & 1)) {  // $oneShot_0008
            if (I_SerialReady) {  // $SerialReady
                Modbus_Send(1, 0UL, MODBUS_FC_READ_HOLDING_REGISTERS, 0, 1, &ArrayIntUser[6]);
    ...
    if (((ArrayBitSystem[0] >> 6) & 1)) {  // $parThis_0000
        ArrayIntUser[6] = ArrayIntUser[6] + ArrayIntUser[2]; // SaidasPOP2 = SaidasPOP2 + Incremento
    }
    ...
    if (((ArrayBitSystem[0] >> 6) & 1)) {  // $parThis_0000
        if (!((ArrayBitSystem[0] >> 19) & 1)) {  // $oneShot_000b
            if (I_SerialReady) {  // $SerialReady
                Modbus_Send(0, 0UL, MODBUS_FC_WRITE_MULTIPLE_REGISTERS, 0, 1, &ArrayIntUser[6]);
    ...
    if(((ArrayBitSystem[0] >> 6) & 1)) ArrayBitSystem[0] |= 1UL << 10; else ArrayBitSystem[0] &= ~(1UL << 10); // $parThis_0001 = $parThis_0000
    if (((ArrayBitSystem[0] >> 10) & 1)) {  // $parThis_0001
        if (ArrayIntUser[7] < 99) {
            ArrayIntUser[7]++;
            ArrayBitSystem[0] &= ~(1UL << 10); // $parThis_0001 = 0
        }
    } else {
        ArrayIntUser[7] = 0; // EsperaSync
    }
    ...
    /* iniciando serie [ */
    if (((ArrayBitUser[0] >> 0) & 1)) {  // IncSync
        ArrayBitSystem[0] &= ~(1UL << 1); // $rung_top = 0
    }

    if (((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        if (ArrayIntUser[8] < 299) {
            ArrayIntUser[8]++;
            ArrayBitSystem[0] &= ~(1UL << 1); // $rung_top = 0
        }
    } else {
        ArrayIntUser[8] = 0; // EspON
    }

    if (((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        if (!((ArrayBitSystem[0] >> 23) & 1)) {  // $oneShot_000d
            if (I_TcpReady) {  // $TcpReady
                Modbus_Send(1, 3232236262UL, MODBUS_FC_READ_HOLDING_REGISTERS, 0, 1, &ArrayIntUser[2]);
                ArrayBitSystem[0] |= 1UL << 23; // $oneShot_000d = 1
            }
            ArrayBitSystem[0] &= ~(1UL << 1); // $rung_top = 0
            ArrayBitSystem[0] &= ~(1UL << 24); // $oneShot_000e = 0
        }
        if (!((ArrayBitSystem[0] >> 24) & 1)) {  // $oneShot_000e
            ArrayBitSystem[0] &= ~(1UL << 1); // $rung_top = 0
            if (I_TcpReady) {  // $TcpReady
                ArrayBitSystem[0] |= 1UL << 24; // $oneShot_000e = 1
        ...
        ArrayBitSystem[0] &= ~(1UL << 23); // $oneShot_000d = 0
        ArrayBitSystem[0] &= ~(1UL << 24); // $oneShot_000e = 0
    }

    if (!((ArrayBitSystem[0] >> 25) & 1)) {  // $EspOFF_antiglitch
        ArrayIntUser[9] = 299; // EspOFF
    }
    ArrayBitSystem[0] |= 1UL << 25; // $EspOFF_antiglitch = 1
    if (!((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        if (ArrayIntUser[9] < 299) {
            ArrayIntUser[9]++;
            ArrayBitSystem[0] |= 1UL << 1; // $rung_top = 1
        }
    } else {
        ArrayIntUser[9] = 0; // EspOFF
    }

    if(((ArrayBitSystem[0] >> 1) & 1)) ArrayBitUser[0] |= 1UL << 0; else ArrayBitUser[0] &= ~(1UL << 0); // IncSync = $rung_top
    ...
    if(((ArrayBitSystem[0] >> 1) & 1)) ArrayBitSystem[0] |= 1UL << 3; else ArrayBitSystem[0] &= ~(1UL << 3); // $scratch = $rung_top
    if (!((ArrayBitSystem[0] >> 1) & 1)) {  // $rung_top
        if (((ArrayBitSystem[0] >> 26) & 1)) {  // $oneShot_000f
            ArrayBitSystem[0] |= 1UL << 1; // $rung_top = 1
        }
    } else {
        ArrayBitSystem[0] &= ~(1UL << 1); // $rung_top = 0
    }
    if(((ArrayBitSystem[0] >> 3) & 1)) ArrayBitSystem[0] |= 1UL << 26; else ArrayBitSystem[0] &= ~(1UL << 26); // $oneShot_000f = $scratch
...
void PLC_Init(void)
{
	...
	IP4_ADDR(&IP_GATEWAY, 192,168,2,10);
	IP4_ADDR(&IP_DNS, 192,168,2,1);
